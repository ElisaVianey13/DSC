...
data1 = read.csv("D:/KAMPUS/DSC 2020/datatugas6.csv", sep=";", header = T)
View(data1)
dim(data1)
head(data1)
ylt=as.ts(data1[,2])
...
#LOAD THE PACKAGE TO ,AKE DUMMY VARIABLES
library(dummies)

#DUMMY VARIABLES TOR TREND
Dummy.T =c(1:168)
head(Dummy.T)
tail(Dummy.T)
...
c=min(min(fore.TSR), min(lower), min(Ytrain))   #lower bound for testing data
d=max(max(fore.TSR), max(lower), max(Ytrain))   #upper bound for testing data
a
b
c
d
...
#DIAGNOTIC CHECKING FOR ARIMA MODEL
...
accuracies[1,2]=accuracy(fits.TSR, Ytrain)[1,2]
