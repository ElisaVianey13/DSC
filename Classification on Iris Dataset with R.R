...
###Importing Data
iris = read.csv("D:/KAMPUS/DSC 2020/iris.csv", row.names=1)  #Import data
View(iris)
...
str(iris)  #to view structure of the iris dataset
unique(iris[c("Species")])  #to check the class on the species column
table(iris$Species)  #to count the number of obsevations in each class ONLY,  the results also can be seen in 

###Preprocessing Data
#Detecting Missing Value
summary(iris) #if there is no NA'S in then there is no missing value on the dataset

#Detecting Outlier
boxplot(features, main = "Detecting Ouliers on Each variable")  #to check wether there are outlier
...
outlier_df
barplot(outlier_df$Number.of.observations, names.arg = outlier_df$Notes, main = "Detecting Multivariate Outliers")
...
boxplot(features_normalized, main="Detecting Outliers on Each Variable After Normalization") #to check wehter there are still outliers or not
...
### Classifacation using K-Nearest Neighbor
...
#Classifation using K-Nearest Neighbor
set.seed(0)
library(class)
k = 3
