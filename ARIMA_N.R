...
data1 = read.csv("D:/KAMPUS/DSC 2020/PERTEMUAN 11 (ARIMA N)/datatugas6.csv", sep=";", header = T)
View(data1)
ylt =as.ts(data1[,2])
Ytrain =as.ts(ylt[1:144])           #define training data
Ytest=as.ts(ylt[145:168])           #define testing data
...
box()
axis(side=1,at=tick, lwd=0.5, las=0, cex.axis=0.8)
axis(side=2, lwd=0.5, las=2,cex=0.5, cex.axis=0.8)
...
#DIFEERENCE NON-SEASONAL ORDER FOR WTRAIN
Ztrain=diff(Wtrain, lag=1)
par(mfrow=c(1,1))
plot(Ztrain)
adf.test(Ztrain)

#ORDER INDENTIFICATION USING ACF AND PACF FROM STATIONARY DATA
tick=c(1,24,48,72)
...
#CONSTRUCT INTERVAL PREDICITION
...
write.csv(forecast, file="forecast arima 2.csv")
...
axis(side=2, lwd=0.5, cex.axis=0.8, las=0)
axis(side=1, lwd=0.5, cex.axis=0.8, las=0, at=seq(1:24), labels=c(145:168))
